...
默认上限：敌方 Worker 1 名攻击者，敌方 Vanguard/Ranger 2 名攻击者；正在攻击己方 Unit 或
Core 的目标可提高到 3。排序使用 `(目标类型优先级, 距离, 敌方 UUID)`，攻击者使用
`(任务优先级, 距离, Unit 类型, UUID)`。预留只约束主动攻击，不阻止紧急自卫、低 HP 逃跑或
Core 防守。Vanguard 负责相邻屏障，Ranger 只在合法射线和未超过预留时开火。
...
`(start, goal, static_obstacle_signature, blocked_signature)` 构成；只缓存静态障碍未变化且
同一步骤可复用的第一步，动态占位变化时自然失效。缓存不跨 Tick、不写磁盘。
...
`plan_turn()` 每 Tick 计算一次友军视野来源和覆盖结果，`search_goal_for()` 只消费缓存，不
为每个搜索任务再次遍历全部 Unit。缓存键包含来源位置、视野半径、障碍签名和任务中心；任务
只读取执行小队所需区域时，优先使用小队覆盖子集。
...
加入 Tick 级路径统计、短生命周期缓存和搜索覆盖缓存。验收：缓存命中、A* 扩展数和决策耗时
进入日志；缓存失效后仍返回与无缓存版本相同的合法第一步；40/60 人口压力测试无长期卡死。
