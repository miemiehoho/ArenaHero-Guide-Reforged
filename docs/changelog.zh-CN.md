...
- 新增 [资源采集与攻击编队分散优化详细设计](Arena-Hero-Guide-Reforged资源采集与攻击编队分散优化详细设计.md)，记录官方约束、状态兼容、分阶段范围和验收结果。
- 阶段 A（`36fbdf0`）让当前 Tick 可见资源直接进入分配候选；Worker 远程搜索避开本 Tick
  友军占位，A* 首步失败时换环点，避免长期 `wait-scout`。
- 阶段 B（`e02ee16`）让 Rally 只服务活动攻击波，非活动波继续独立巡逻；不同小队按稳定编号
  轮换攻击入口，Ranger 保持合法射击位和后侧分工。
- 未新增持久化字段，现有 v10 状态文件无需迁移；离线 unittest 从 165 项增加到 171 项，
