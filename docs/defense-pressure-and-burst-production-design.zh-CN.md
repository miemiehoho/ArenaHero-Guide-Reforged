...
实际可自毁数不足时缩小上界。Worker 选择按“空载优先、Cargo 少优先、UUID 稳定顺序”确定，
避免为了相同收益丢弃更多在途资源。
...
新增本文和 README 索引，验证官方事实、链接和格式后独立提交推送。

### B1：危险评估与诊断

只实现纯危险评估、规划上下文和诊断，不改变生产或自毁行为。

### B2：动态价格优化器

实现纯经济模拟和候选选择，不提交 Worker 自毁动作。

### B3：动作集成

接入 `CRITICAL` 生产优先级、Worker 自毁和 Core 生产，覆盖官方动作边界。

### D2：文档验收

更新 README、本文状态和更新日志，记录提交号、最终测试数量及完整验收结果。
