...
截至 2026-08-16，40+ 人口优化四阶段、官方规则对齐 A1～A4、中文日志体系、Worker
分区探索与战损补员以及受击危险评估与爆兵优化均已完成，离线测试共 165 个。
...
- 每 Tick 按 Core 12 格内的敌方战斗单位、本地防守数量、Core 损伤和 6 Tick 敌情记忆，将
  近家危险分为 `NONE`、`PRESSURED`、`CRITICAL`；`PRESSURED` 只召回迎击，`CRITICAL`
  才绕过未满仓门槛紧急生产战斗单位。
- `CRITICAL` 时会使用官方 `unit_cost()` 与 `core_resource_capacity()` 比较直接生产和自毁部分
  Worker 后的动态价格方案；只有后者能严格多生产战斗单位时才自毁，且至少保留 12 名 Worker，
  不选择 Champion Beacon 携带者。Core 恢复、迁移和生产格清理仍优先。
- Core 可以由用户手动迁移；迁移后会重置依赖旧家位置的侦察、撤退和巡逻目标。
- 持久化状态版本为 10；v9 的扇区和敌情状态会迁移到 v10，v8 会保留敌方 Core 和攻击状态
  并继续迁移，v7 及更早状态不会携带旧敌方 Core 或攻击状态。Core 换家后仍保留世界坐标下的
  敌方 Core 与彻查任务。
- 控制模式正常发展固定生产 4 名 Worker，其余人口严格按 `2 Vanguard + 1 Ranger` 编成小队。
  曾达到 20 人口后若发生战损并导致人口下降，会跳过满仓门槛进入补员：先把 Worker 补到至少
  12 名，再按现有小队规则恢复 Vanguard/Ranger；未发生战损时仍保持原有 4 Worker 和满仓扩编策略。
...
- 集结时只有活动波使用 Rally；未参与当前波次的小队继续各自巡逻。攻击推进按小队编号轮换
  敌方目标接近格，减少多个小队挤在同一入口。
...
- Ranger 跟随本队 Vanguard 并优先处于行军方向后侧；敌方 Vanguard 贴身时先脱离，
  不会站在队伍最前方与近战单位硬换血。
- Ranger 的攻击位优先保持 1～3 格合法射线，并排除己方 Core；搜索首步受临时占位阻挡时，
  Worker 会换用同一搜索序列的下一环点，只有全序列不可达才记录 `scout-blocked`。
...
确认该格为空时才会删除。资源池基础范围为当前 Core 切比雪夫距离 36 格，并随 Worker 数量的
普通搜索范围扩展到 40、48、56、64 格；满仓外圈模式至少保留 64 格，保证远端发现的资源不会
因越过旧边界而被丢弃。Core 迁移后会立即清除范围外资源以及相关 Worker 任务。

普通资源搜索状态下，没有资源任务的 Worker 会在方形环上顺时针扫描：1～4 名 Worker 使用
`12 → 19 → 26 → 32`，5～8 名增加 40 格环，9～12 名增加 48 格环，13～16 名增加 56 格环，
17 名以上最大扩展到 64 格，再按同样顺序向内返回。Worker 按稳定 UUID 和实际数量分配连续扇区，
12 名时不会复用目标区域；目标被障碍占据时跳过，连续三次无法寻路时也会推进到下一个目标。
资源任务、载货返航、威胁逃跑和治疗均保持更高优先级。
