...
版本为 v1.0；防守基线 `ecf5b65` 已保存为 annotated Tag `固若金汤正式版v1`，可直接用于回滚。
...
4. 高库存外圈模式下执行 32-64 格方环扫描；Core 已满时，无法交付的载货 Worker 也参加。
5. 非外圈模式且 Core 满仓时分散待命并腾空生产格。
6. 采集脚下资源。
7. 前往已静态分配的资源。
8. 无资源任务时协助拦截敌方 Worker。
9. 按固定扇区侦察或等待。
