...
迎敌；巡逻单位根据局部兵力选择撤退或攻击。Core 周围曼哈顿距离 18 格内出现敌方
Vanguard/Ranger 时，游走单位优先驰援，再处理追击 Worker、搭档跟随和普通巡逻。

控制模式达到 19 人、8 名 Worker、Core 资源至少 80，且家园 18 格内没有敌方战斗单位时，
会从游走巡逻单位中选择距离最紧凑的
`2 Vanguard + 1 Ranger` 组成冠军信标远征队。远征队保持 5 格以内的编队，沿途清扫
8 格内的敌方 Unit，单个目标最多追击 8 Tick，并持续跟随公开的信标坐标。抵达信标且
附近没有清扫目标后不拾取信标，整队按原编队和战斗规则直接返航，到家后恢复巡逻。

远征队减员后不再向前线逐个补兵，幸存成员立即返航并进入 300 Tick 冷却。普通生产先恢复
人口和战斗编成；冷却结束且再次满足完整启动条件后，才会重新抽调一支完整小队。此分支
不会主动拾取冠军信标，包括信标落在家园附近的情况。
